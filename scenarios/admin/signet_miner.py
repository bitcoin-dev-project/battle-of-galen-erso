...
def do_createpsbt(block, signme, spendme):
    ...
    psbt.g = PSBTMap(
        {
            PSBT_GLOBAL_UNSIGNED_TX: signme.serialize(),
...
def do_decode_psbt(b64psbt):
    ...
    return from_binary(CBlock, psbt.g.map[PSBT_SIGNET_BLOCK]), ser_string(
        scriptSig
    ) + scriptWitness
...
def generate_psbt(tmpl, reward_spk, *, blocktime=None):
    ...
    cbtx = create_coinbase(
        height=tmpl["height"], value=tmpl["coinbasevalue"], spk=reward_spk
    )
...
def get_reward_address(args, height):
    ...
    if addr is None:
        addrs = args.bcli(
            "deriveaddresses", args.descriptor, "[%d,%d]" % (height, height + 20)
        )
        addr = addrs[0]
...
def do_generate(args):
    ...
    if args.set_block_time is not None and max_blocks != 1:
        logging.error(
            "Cannot specify --ongoing or --max-blocks > 1 when using --set-block-time"
        )
        return 1
    if args.set_block_time is not None and args.set_block_time < 0:
        args.set_block_time = time.time()
        logging.info(
            "Treating negative block time as current time (%d)" % (args.set_block_time)
        )
    ...
    if args.set_block_time is None:
        if args.nbits is None or len(args.nbits) != 8:
            logging.error(
                "Must specify --nbits (use calibrate command to determine value)"
            )
            return 1
    ...
    while max_blocks is None or mined_blocks < max_blocks:
        ...
        if args.set_block_time is not None:
            ...
            logging.info(
                "Backdating time for first block to %d minutes ago" % (time_delta / 60)
            )
        ...
        if now < action_time:
            ...
            logging.debug(
                "Sleeping for %s, next block due in %s (%s)"
                % (seconds_to_hms(sleep_for), seconds_to_hms(mine_time - now), minestr)
            )
        ...
        if tmpl["mintime"] > mine_time:
            logging.info(
                "Updating block time from %d to %d", mine_time, tmpl["mintime"]
            )
        ...
        psbt_signed = args.bcli(
            "walletprocesspsbt", psbt=psbt, sign=True, sighashtype="ALL"
        )
        ...
        if r != "":
            logging.warning(
                "submitblock returned %s for height %d hash %s",
                r,
                tmpl["height"],
                block.hash,
            )
...
def do_calibrate(args):
    ...
    print(
        "nbits=%08x for %ds average mining time"
        % (target_to_nbits(want_targ), want_time)
    )
    return 0
...
def get_args(parser):
    parser.add_argument(
        "--cli", default="bitcoin-cli", type=str, help="bitcoin-cli command"
    )
    parser.add_argument("--debug", action="store_true", help="Print debugging info")
    parser.add_argument(
        "--quiet", action="store_true", help="Only print warnings/errors"
    )
    ...
    generate.add_argument(
        ...
        default=None,
        type=int,
        help="Set block time (unix timestamp)",
    )
    generate.add_argument(
    ...
    generate.add_argument(
        "--min-nbits",
        action="store_true",
        help="Target minimum nBits (use min difficulty)",
    )
    generate.add_argument(
        "--poisson", action="store_true", help="Simulate randomised block times"
    )
    generate.add_argument(
        "--multiminer",
        default=None,
    ...
    generate.add_argument(
        ...
        default=1800,
        type=int,
        help="Maximum interblock interval (seconds)",
    )
    ...
    for sp in [genpsbt, generate]:
        ...
        sp.add_argument(
            ...
            default=None,
            type=str,
            help="Descriptor for block reward payment",
        )
